...
public class Local {
	...
	public static Integer[] getYears() {
		...
		for (int i = 0; i < years.length; i++) {
			years[i] = Calendar.getInstance().get(Calendar.YEAR) + i - 5;
		}
		return years;
	}
	...
	public static String getDateString(Calendar cal, int f) {
		...
		/*
		 * String s = getMonthName(cal.get(Calendar.MONTH)) + " " +
		 * cal.get(Calendar.DAY_OF_MONTH) + ", " + cal.get(Calendar.YEAR) + " (" +
		 * getWeekdayName(cal.get(Calendar.DAY_OF_WEEK)) + ")"; return s;
		 */
	...
	public static String getTimeString(Calendar cal) {
		/*
		 * String h = String.valueOf(cal.get(Calendar.HOUR_OF_DAY)); if (h.length() < 2)
		 * { h = "0" + h; } String m = String.valueOf(cal.get(Calendar.MINUTE)); if
		 * (m.length() < 2) { m = "0" + m; } return h + ":" + m;
		 */
	...
	public static String getTimeString(int hh, int mm) {
		 ...
		 * = String.valueOf(mm); if (m.length() < 2) { m = "0" + m; } return h + ":" +
		 * m;
		 */
		Calendar cal = Calendar.getInstance();
