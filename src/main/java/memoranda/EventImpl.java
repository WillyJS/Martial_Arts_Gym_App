...
public class EventImpl implements Event, Comparable {
    ...
    // Added a method to get the trainer attribute US29 PD
    public String getTrainer() {
        Attribute trainer = _elem.getAttribute("trainer");
        if (trainer != null)
    ...
    public String getLevel() {
        Attribute level = _elem.getAttribute("level");
        if (level != null)
    ...
    public String getRoom() {
        Attribute room = _elem.getAttribute("room");
        if (room != null)
    ...
    public String getDojoId() {
        Attribute dojoId = _elem.getAttribute("dojoId");
        if (dojoId != null)
            return dojoId.getValue();
        return null;
    }
    ...
    public String getClassType() {
        Attribute classType = _elem.getAttribute("classType");
        if (classType != null)
    ...
    public Date getTime() {
        ...
        Date d = new Date(); // Revision to fix deprecated methods (jcscoobyrs) 12-NOV-2003
                             // 14:26:00
        Calendar calendar = new GregorianCalendar(Local.getCurrentLocale()); // Revision to fix
                                                                             // deprecated methods
                                                                             // (jcscoobyrs)
                                                                             // 12-NOV-2003
                                                                             // 14:26:00
        calendar.setTime(d); // Revision to fix deprecated methods (jcscoobyrs) 12-NOV-2003
                             // 14:26:00
        calendar.set(Calendar.HOUR_OF_DAY, getHour()); // Revision to fix deprecated methods
                                                       // (jcscoobyrs) 12-NOV-2003
                                                       // 14:26:00
        calendar.set(Calendar.MINUTE, getMinute()); // Revision to fix deprecated methods
                                                    // (jcscoobyrs) 12-NOV-2003
                                                    // 14:26:00
        calendar.set(Calendar.SECOND, 0); // Revision to fix deprecated methods (jcscoobyrs)
                                          // 12-NOV-2003 14:26:00
        d = calendar.getTime(); // Revision to fix deprecated methods (jcscoobyrs) 12-NOV-2003
                                // 14:26:00
        return d;
    }
