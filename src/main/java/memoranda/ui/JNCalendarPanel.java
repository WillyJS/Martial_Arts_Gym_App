...
public class JNCalendarPanel extends JPanel {
	...
	// New year drop down menu PD US4
	JComboBox yearDropDownBox = new JComboBox(Local.getYears());
	...
	public Action dayBackAction = new AbstractAction("Go one day back",
	...
	public Action dayForwardAction = new AbstractAction("Go one day forward",
	...
	public Action todayAction = new AbstractAction("Go to today",
	...
	void jbInit() throws Exception {
		// dayBackAction.putValue(Action.ACCELERATOR_KEY,
		// KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_DOWN, KeyEvent.ALT_MASK));
		// dayForwardAction.putValue(Action.ACCELERATOR_KEY,
		...
		// New Year pull down menu PD US4
		yearDropDownBox.setRequestFocusEnabled(false);
		yearDropDownBox.setMaximumRowCount(15);
		yearDropDownBox.setPreferredSize(new Dimension(70, 20));
		...
		jnCalendar.getTableHeader().setFont(new java.awt.Font("Dialog", 1, 12));
		jnCalendar.setFont(new java.awt.Font("Dialog", 0, 15));
		jnCalendar.setGridColor(Color.lightGray);
		jnCalendar.setRowHeight(25); //Increased the row height to improve visibility in US2Task13
		...
		mntyPanel.add(yearDropDownBox, BorderLayout.EAST);
		...
		/*
		 * CurrentDate.addChangeListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { _date = CurrentDate.get(); refreshView(); }
		 * });
		 */
		...
		yearDropDownBox.setFont(new java.awt.Font("Dialog", 0, 11));

		yearDropDownBox.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				yearDropDownBox_actionPerformed(e);
			}
		});
	...
	private void refreshView() {
		...
		yearDropDownBox
				.setSelectedIndex(new Integer(_date.getYear() - (Calendar.getInstance().get(Calendar.YEAR) - 5)));
	...
	void yearDropDownBox_actionPerformed(ActionEvent e) {
		if (ignoreChange) {
			return;
		}
		_date = new CalendarDate(_date.getDay(), _date.getMonth(),
				yearDropDownBox.getSelectedIndex() + Calendar.getInstance().get(Calendar.YEAR) - 5);
		jnCalendar.set(_date);
		notifyListeners();
	}
