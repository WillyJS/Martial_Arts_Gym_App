...
public class EventsManager {
    ...
    // Create an arrayList of dojo classes US70 PD
    public static ArrayList<DojoClass> dojoClasses = new ArrayList<DojoClass>();
    ...
    static {
        ...
        if (_doc == null) {
            ...
            /*
             * _root.addNamespaceDeclaration("jnevents", NS_JNEVENTS); _root.appendChild(
    ...
    public static Event createEvent(CalendarDate date, int hh, int mm, String text, String trainer,
            String student, int room, String level, int dojoId, String classType) {
        ...
        // Append the trainer to the event element US29 PD US70 PD
        el.addAttribute(new Attribute("trainer", trainer));
        el.addAttribute(new Attribute("room", String.valueOf(room)));
        el.addAttribute(new Attribute("level", level));
        el.addAttribute(new Attribute("dojoId", String.valueOf(dojoId)));
        el.addAttribute(new Attribute("classType", classType));
    ...
    public static Event createRepeatableEvent(int type, CalendarDate startDate,
            CalendarDate endDate, int period, int hh, int mm, String text, String trainer,
            boolean workDays) {
        Element el = new Element("event");
        ...
        // Append the trainer to the repeatable event element US29 PD
        el.addAttribute(new Attribute("trainer", trainer));
    ...
    public static Collection getRepeatableEventsForDate(CalendarDate date) {
        ...
        for (int i = 0; i < reps.size(); i++) {
            ...
            // ---
            /*
             * /if ( ((date.after(ev.getStartDate())) && (date.before(ev.getEndDate()))) ||
            ...
            if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
                if (ev.getRepeat() == REPEAT_DAILY) {
                    ...
                    int ns = ev.getStartDate().getCalendar().get(Calendar.DAY_OF_YEAR);
    ...
    public static Event getEvent(CalendarDate date, int hh, int mm) {
        ...
        for (int i = 0; i < els.size(); i++) {
            ...
            if ((new Integer(el.getAttribute("hour").getValue()).intValue() == hh)
    ...
    static class Month {
        ...
        private Day createDay(int d) {
            ...
            el.addAttribute(
                    new Attribute("date",
                            new CalendarDate(d, getValue(),
                                    new Integer(((Element) mElement.getParent())
                                            .getAttribute("year").getValue()).intValue())
                                                    .toString()));
     ...
     * 
     * private static void doSort(int L, int R) { // Hoar's QuickSort int i = L; int
     * j = R; int x = toMinutes(keys.get((L + R) / 2)); Object w = null; do { while
     * (toMinutes(keys.get(i)) < x) { i++; } while (x < toMinutes(keys.get(j))) {
     * j--; } if (i <= j) { w = keys.get(i); keys.set(i, keys.get(j)); keys.set(j,
     * w); i++; j--; } } while (i <= j); if (L < j) { doSort(L, j); } if (i < R) {
     * doSort(i, R); } }
     * 
